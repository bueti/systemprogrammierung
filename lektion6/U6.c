...
void *nonblocking(void *arg) {
    ...
    // handle_thread_error(retcode, "child failed (timed)lock", PROCESS_EXIT);
    printf("%d: in nonblocking thread: got mutex\n", my_tidx);

    /* replace with char line[1025]; fgets(line, 1024, stdin) */
    char line[1025];
    //fgets(line, 1024, stdin);
    printf("%s", fgets(line, 1024, stdin));

    printf("%d: in nonblocking thread: releasing mutex\n", my_tidx);
    pthread_mutex_unlock(&mutex);
    printf("%d: in nonblocking thread: released mutex\n", my_tidx);
    printf("%d: in nonblocking thread: exiting\n", my_tidx);
    return NULL;
}
...
int main(int argc, char *argv[]) {
    ...
    printf("in parent: setting up threads\n");
    ...
    for (size_t i = 0; i < N_THREADS; i++) {
        tidx[i] = (int) i;
        if (i%2) {
            retcode = pthread_create(thread + i, NULL, blocking, (void *) (tidx + i));
        } else {
            retcode = pthread_create(thread + i, NULL, nonblocking, (void *) (tidx + i));
        }
    }
